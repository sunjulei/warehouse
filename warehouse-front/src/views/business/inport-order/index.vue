  ...
  <div>
    <el-card>
      <SearchForm v-model="searchParams" @search="handleSearch" @reset="handleReset">
        <el-form-item label="供应商">
          <el-select v-model="searchParams.providerid" placeholder="全部" clearable filterable style="width: 200px">
    ...
    <el-dialog v-model="returnDialogVisible" title="退货" width="900px" @close="resetReturnForm">
      ...
      <div v-if="returnMode === 'single'" class="return-single">
        <el-table :data="returnableItems" border style="width: 100%">
          ...
          <el-table-column label="退货金额" width="100" align="right">
            <template #default="{ row }">
              <span style="color: #f56c6c; font-weight: 600;">¥{{ (row.inportprice * (row.returnQty || 0)).toFixed(2) }}</span>
            </template>
          </el-table-column>
        </el-table>
      ...
      <div v-if="returnMode === 'all'" class="return-all">
        <el-alert title="整单退货将退还未退货的所有商品，退货后将扣减所有商品库存。" type="warning" :closable="false" show-icon />
      ...
      <div v-for="(item, index) in addForm.items" :key="index" class="add-item-row">
        <el-form-item :label="'商品' + (index + 1)" class="add-item-form">
          <div class="add-item-content">
            ...
            <el-input-number v-model="item.inportprice" :min="0" :precision="2" placeholder="进货价" style="width: 120px" />
            <span class="add-item-subtotal">¥{{ (item.number * item.inportprice).toFixed(2) }}</span>
...
const mergedOrderDetails = computed(() => {
  ...
  for (const item of orderDetails.value) {
    ...
    const key = `${item.goodsid}_${item.inportprice}`
    if (map.has(key)) {
...
const returnSingleTotalAmount = computed(() => {
  return returnableItems.value.reduce((sum, item) => {
    return sum + item.inportprice * (item.returnQty || 0)
  }, 0)
})
...
const orderDetailTotalAmount = computed(() => {
  return mergedOrderDetails.value.reduce((sum, item) => sum + item.inportprice * item.number, 0)
})
...
const printMeta = computed<PrintMetaItem[]>(() => [
  { label: '供应商', value: currentOrder.value?.providerName },
  { label: '进货时间', value: currentOrder.value?.inporttime },
...
const printColumns: PrintColumn[] = [
  ...
  { key: 'inportprice', label: '进货价', align: 'right', width: '90px', format: (row) => `¥${row.inportprice?.toFixed(2)}` },
  { key: 'subtotal', label: '小计', align: 'right', width: '100px', format: (row) => `¥${(row.inportprice * row.number).toFixed(2)}` }
]
...
const returnAllTotalAmount = computed(() => {
  return returnableItems.value.reduce((sum, item) => sum + item.inportprice * item.number, 0)
})
...
const addTotalAmount = computed(() => {
  return addForm.items.reduce((sum, item) => sum + (item.number || 0) * (item.inportprice || 0), 0)
})

const handleSearch = () => tableRef.value?.reload()
const handleReset = () => { searchParams.providerid = null }
  ...
  try {
    if (returnMode.value === 'single') {
      ...
      for (const item of itemsToReturn) {
        ...
        for (const inportId of item.ids) {
          if (remaining <= 0) break
          const detail = orderDetails.value.find((d: any) => d.id === inportId)
          if (!detail || detail.number <= 0) continue
          const qty = Math.min(remaining, detail.number)
          await returnSingleGoods(inportId, qty)
          remaining -= qty
        }
      }
...
const addAddItem = () => {
  addForm.items.push({ goodsid: null, number: 1, inportprice: 0 })
}
...
const onGoodsChange = (index: number, goodsid: number) => {
  ...
  if (goods) {
    addForm.items[index].inportprice = goods.price || 0
  }
}
