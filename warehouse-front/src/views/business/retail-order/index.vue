  ...
  <div>
    <el-card>
      <SearchForm v-model="searchParams" @search="handleSearch" @reset="handleReset">
        <el-form-item label="支付方式">
          <el-select v-model="searchParams.paytype" placeholder="全部" clearable style="width: 150px">
      ...
      <CrudTable ref="tableRef" :load-api="loadAllOrders" :search-params="searchParams" :row-class-name="tableRowClassName">
        ...
        <el-table-column prop="operateperson" label="操作员" min-width="100" />
        <el-table-column prop="remark" label="备注" min-width="150" show-overflow-tooltip>
          <template #default="{ row }">
            {{ row.remark ? (row.remark.length > 15 ? row.remark.substring(0, 15) + '...' : row.remark) : '' }}
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="detailDialogVisible" title="订单详情" width="800px">
      <div class="order-detail-header" v-if="currentOrder">
        ...
        <span>支付方式: <strong>{{ currentOrder.paytype }}</strong></span>
        <span>时间: <strong>{{ currentOrder.retailtime }}</strong></span>
      ...
      <el-table :data="mergedOrderDetails" border style="width: 100%">
        ...
        <el-table-column prop="retailprice" label="单价" width="100" align="right">
          <template #default="{ row }">
      ...
      <div v-if="returnMode === 'single'" class="return-single">
        <el-table :data="returnableItems" border style="width: 100%">
          ...
          <el-table-column label="退货金额" width="100" align="right">
            <template #default="{ row }">
              <span style="color: #f56c6c; font-weight: 600;">¥{{ (row.retailprice * (row.returnQty || 0)).toFixed(2) }}</span>
            </template>
          </el-table-column>
        </el-table>
      ...
      <div v-for="(item, index) in addForm.items" :key="index" class="add-item-row">
        <el-form-item :label="'商品' + (index + 1)" class="add-item-form">
          <div class="add-item-content">
            ...
            <el-input-number v-model="item.retailprice" :min="0" :precision="2" placeholder="售价" style="width: 120px" />
            <span class="add-item-subtotal">¥{{ (item.number * item.retailprice).toFixed(2) }}</span>
...
const mergedOrderDetails = computed(() => {
  ...
  for (const item of orderDetails.value) {
    ...
    const key = `${item.goodsid}_${item.retailprice}`
    if (map.has(key)) {
...
const returnSingleTotalAmount = computed(() => {
  return returnableItems.value.reduce((sum, item) => {
    return sum + item.retailprice * (item.returnQty || 0)
  }, 0)
})
...
const orderDetailTotalAmount = computed(() => {
  return mergedOrderDetails.value.reduce((sum, item) => sum + item.retailprice * item.number, 0)
})
...
const returnAllTotalAmount = computed(() => {
  return returnableItems.value.reduce((sum, item) => sum + item.retailprice * item.number, 0)
})
...
const addTotalAmount = computed(() => {
  return addForm.items.reduce((sum, item) => sum + (item.number || 0) * (item.retailprice || 0), 0)
})

const handleSearch = () => tableRef.value?.reload()
const handleReset = () => { searchParams.paytype = null }
  ...
  try {
    if (returnMode.value === 'single') {
      ...
      for (const item of itemsToReturn) {
        ...
        for (const retailId of item.ids) {
          if (remaining <= 0) break
          const detail = orderDetails.value.find((d: any) => d.id === retailId)
          if (!detail || detail.number <= 0) continue
          const qty = Math.min(remaining, detail.number)
          await returnSingleGoods(retailId, qty)
          remaining -= qty
        }
      }
...
const addAddItem = () => {
  addForm.items.push({ goodsid: null, number: 1, retailprice: 0 })
}
...
const onGoodsChange = (index: number, goodsid: number) => {
  ...
  if (goods) {
    addForm.items[index].retailprice = goods.price || 0
  }
}
